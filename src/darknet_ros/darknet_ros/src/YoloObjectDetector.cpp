...
//Custom Message
#include "darknet_ros/Labels.h"
#include "darknet_ros/Label.h"
#include "darknet_ros/Point.h"
...
void YoloObjectDetector::init() {
  ...
  labelPublisher_ =
      nodeHandle_.advertise<darknet_ros::Labels>("inference_yolo_output", 10);
...
void* YoloObjectDetector::publishInThread() {
  ...
  if (num > 0 && num <= 100) {
    ...
    for (int i = 0; i < numClasses_; i++) {
      if (rosBoxCounter_[i] > 0) {
        ...
        for (int j = 0; j < rosBoxCounter_[i]; j++) {
          ...
          darknet_ros::Label contour;
          contour.category = i+1;

          darknet_ros::Point point;
          point.x = xmin;
          point.y = ymin;
          contour.contour.push_back(point);
          ...
          labels.labels.push_back(contour);
        }
      }
    }
    ...
    labelPublisher_.publish(labels);
  } else {
    darknet_ros_msgs::ObjectCount msg;
    msg.header.stamp = ros::Time::now();
    msg.header.frame_id = "detection";
