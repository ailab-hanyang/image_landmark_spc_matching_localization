...
void tfBroadcaster::CallBackNovatelINSPVAX(const novatel_oem7_msgs::INSPVAX::ConstPtr &msg)
{
    ...
    // std::cout << " nov roll : "<<-(msg->roll) * M_PI / 180.<<" pitch : "<<-(msg->pitch) * M_PI / 180.<<std::endl;
    // std::cout << " nov yaw : "<<(-1*msg->azimuth + 90.0) * M_PI<<std::endl;
    ...
    // // brodcast
    // tfbroad_novatel.sendTransform( 
    //     tf::StampedTransform(tf::Transform(quternion_velodyne, offset_velodyne), rostime_pres_time, "novatel", "velodyne"));
}
...
void tfBroadcaster::CallBackEstimatedPose(const geometry_msgs::PoseStamped::ConstPtr &msg)
{
    ...
    // std::cout << " est roll : "<<roll / 180.<<" pitch : "<<pitch<<std::endl;
